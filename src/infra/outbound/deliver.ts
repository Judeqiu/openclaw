...
// Auto-retry configuration for failed deliveries
...
function isRetryableError(err: unknown): boolean {
  ...
  if (
    lowerMessage.includes("429") || // Rate limit
    ...
    // Internal server error (sometimes retryable)
    return true;
  }

  // Telegram specific errors
  if (lowerMessage.includes("retry after") || lowerMessage.includes("too many requests")) {
    return true;
  }
...
export async function deliverOutboundPayloads(
  ...
  // Attempt delivery with automatic retry
  ...
  while (attempt < AUTO_RETRY_MAX_ATTEMPTS) {
    ...
    try {
      ...
      if (attempt < AUTO_RETRY_MAX_ATTEMPTS && isRetryableError(err)) {
        const backoffMs = AUTO_RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1);
        const jitter = Math.random() * 1000; // Add jitter to prevent thundering herd
        const waitMs = backoffMs + jitter;

        console.log(
          `[delivery] Attempt ${attempt}/${AUTO_RETRY_MAX_ATTEMPTS} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        console.log(`[delivery] Retrying in ${Math.round(waitMs)}ms...`);
  ...
  throw new Error(`Delivery failed after ${AUTO_RETRY_MAX_ATTEMPTS} attempts`);
}
